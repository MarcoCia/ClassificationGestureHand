...
from utils import classification as cla
...
def draw_box_on_image(num_hands_detect, score_thresh, scores, boxes, classes, im_width, im_height, image_np, model):
    ...
    for i in range(num_hands_detect):
        if (scores[i] > score_thresh):
            ...
            predizione=cla.prediction(image_np, p1, p2, model)
            dist = distance_to_camera(avg_width, focalLength, int(right-left))

            cv2.rectangle(image_np, p1, p2, color , 3, 1)
            if(id!='closed'):
                cv2.putText(image_np, 'Gesture Recognized: '+str(predizione)+id, (int(left), int(top)-5),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.5 , color, 2)
            else:
                cv2.putText(image_np, 'Gesture NOT Recognized, Hand is '+id, (int(left), int(top)-5),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.5 , color, 2)
